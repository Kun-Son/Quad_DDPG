    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(MAX_STEPS_EPISODE):
            ...
            if i < EXPLORATION_TIME:
                a = (actor4.predict(np.reshape(s, (1, env.observation_space.shape[0]))) + noise[j])[0]
            else:
                a = actor4.predict(np.reshape(s, (1, env.observation_space.shape[0])))[0]
...
def main(_):
    with tf.Session() as sess:
        ...
        saver.save(sess, './results/{}/data_2legs_v3_120_mag_gait'.format(ENV_NAME))
